<template>
  <div class="playlistcontainer">
    ...
    >
      ...
      <a-button type="primary" @click="clearChannel">Clear Channel</a-button>
      <a-button type="primary" @click="createChannel">Create Channel</a-button>
      <a-button type="primary" @click="updateChannel">Uppdate Channel</a-button> -->
          </a-page-header>
        </a-col>
        <a-col :span="4">
        ...
        <template #item="{ element }">
          <div :key="Math.floor(Math.random() * 10100)">
            <a-row class="clip-container">
              <a-col :span="21">{{ element.Name }}</a-col>
              <a-col :span="1">
                {{
                  element.Duration
                    ? convertMillisecondsToDuration(
                        ...
                        `${
                          timestampToMilliseconds(element.Duration) > 360000
                            ? "HHh "
                            : ""
                        } 
                  ${
                    timestampToMilliseconds(element.Duration) > 60000
                      ? "MMm "
                      : ""
                  } 
                  SSs`
    ...
    <div class="playlistsummary">
      <a-row>
        <a-col :span="21">Total duration</a-col>
        <a-col :span="1">{{ totalDuration }}</a-col>
        <a-col :span="2">
          <a-space>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const selectedTimingValue = ref(2);
    const timingoptions = [
      { value: 1, label: "1 sec" },
      { value: 2, label: "2 sec" },
      { value: 3, label: "3 sec" },
    ...
    const totalDuration = computed(() => {
      ...
      return convertMillisecondsToDuration(
        totms,
        `${totms > 360000 ? "HHh " : ""} ${totms > 60000 ? "MMm " : ""} SSs`
      );
    ...
    let updateintervall = setInterval(() => {
      updateChannel();
    }, selectedTimingValue.value * 1000); //TODO Fix a choosable intervall
